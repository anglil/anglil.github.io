...
def process_page(page_img_path, output_images_dir, chapter_name):
    ...
    # Text-Erasure for finding diagrams
    erased_img = img.copy()
    ...
    # We also need a binary threshold of the original image to test the "long stroke" heuristic
    og_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    _, og_thresh = cv2.threshold(og_gray, 200, 255, cv2.THRESH_BINARY_INV)
    ...
    for cnt in contours:
        ...
        # Stroke Heuristic: check if there are any long strokes inside this bounding box
        crop_thresh = og_thresh[max(0, y-5):min(H, y+h+5), max(0, x-5):min(W, x+w+5)]
        sub_contours, _ = cv2.findContours(crop_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        max_sub_w = 0
        max_sub_h = 0
        for scnt in sub_contours:
            _, _, sw, sh = cv2.boundingRect(scnt)
            if sw > max_sub_w: max_sub_w = sw
            if sh > max_sub_h: max_sub_h = sh
            
        # If the largest stroke is smaller than 100x100, it's just math/text words that survived erasure!
        if max_sub_w < 100 and max_sub_h < 100:
            continue # NOT a diagram!
        
        # It's a genuine diagram (graph/plot/illustration)
        pad = 10
        dx = max(0, x - pad)
        dy = max(0, y - pad)
        dw = min(W - dx, w + 2*pad)
    ...
    mkernel = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 5))
