...
def append_page_to_html(page_num, page_html):
    ...
    # Check if page is already in the file (resumability)
    marker = f'id="page-{page_num}"'
    if marker in content:
        print(f"Page {page_num} already in index.html, skipping append.")
        return False
    ...
    if footer_marker in content:
        ...
        with open(html_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Appended Page {page_num} to index.html successfully.")
        return True
    else:
        # Fallback to appending inside main
        if '</main>' in content:
            parts = content.split('</main>', 1)
            new_content = parts[0] + page_html + '\n  </main>' + parts[1]
            with open(html_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"Appended Page {page_num} to index.html (fallback).")
            return True
        else:
            print("Error: Could not find insert location in index.html.")
            return False

def check_resumed_page(page_num):
    html_path = 'college-electrical-engineering/index.html'
    if not os.path.exists(html_path):
        return False
    with open(html_path, 'r', encoding='utf-8') as f:
        content = f.read()
    return f'id="page-{page_num}"' in content
...
def run_batch():
    ...
    # Process starting with the 4th page (index 3) to the end of the PDF
    for page_idx in range(3, total_pages):
        page_num = page_idx + 1
        print(f"\n--- Processing Page {page_num}/{total_pages} (index {page_idx}) ---")
        
        # Check if already processed
        if check_resumed_page(page_num):
        ...
        temp_img_path = f"temp_page_process.png"
        ...
        for cnt in contours_sorted:
            ...
            # Skip if it is too close to full page size (e.g. outer frame)
            if area > (img.shape[0] * img.shape[1] * 0.8):
                continue
                
            # Filter large components that could be diagrams (height > 80, area > 12000)
        ...
        for idx, (x, y, w, h) in enumerate(candidates):
            # Crop original color image with slight padding
            pad = 10
            x1 = max(0, x - pad)
            y1 = max(0, y - pad)
            ...
            crop_path = f"temp_candidate_crop.png"
            cv2.imwrite(crop_path, crop)
            
            # Ask VLM to classify
            prompt_text = 'Is this image purely lines of handwritten text and equations? Or does it contain circuit drawings (like circles with signs/arrows, coordinate axes, brackets)? Answer strictly with TEXT or DIAGRAM.'
            ...
            try:
                ...
                if 'DIAGRAM' in ans and 'TEXT' not in ans:
                    # Save verified diagram
                    dest_filename = f"page_{page_num}_diag_{diag_count}.png"
...
"""
        ...
        # Remove any hallucinated TikZ code
        transcribed = re.sub(r'\\begin\{tikzpicture\}.*?\\end\{tikzpicture\}', '', transcribed, flags=re.DOTALL)
        
        # Replace [DIAGRAM i] placeholders with actual HTML tags
        used_diagrams = set()
        ...
        for block in blocks:
            ...
            if not block:
                continue
            # Check if block is a heading or an image block
            if block.startswith('###'):
        ...
        # Append any unused diagrams at the bottom of the page container as a fallback
        ...
        append_page_to_html(page_num, page_html)
        
        # Clean up page image
        if os.path.exists(temp_img_path):
            os.remove(temp_img_path)
            
        # 7. Clear metal cache to avoid memory leaks
