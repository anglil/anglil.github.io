...
# Initialize models
print("Initializing LatexOCR...", flush=True)
model = LatexOCR()
print("LatexOCR initialized.", flush=True)
...
def rect_intersection(r1, r2):
    ...
    y_bottom = min(r1[1] + r1[3], r2[1] + r2[3])
...
def process_page(page_img_path, output_images_dir, chapter_name):
    ...
    # 2. Perfect Diagram Isolation using Text-Erasure
    erased_img = img.copy()
    for block in ocr_blocks:
        rect = block["full_bbox"]
        x = int(rect.origin.x * W)
        y = int((1.0 - rect.origin.y - rect.size.height) * H)
        w = int(rect.size.width * W)
        h = int(rect.size.height * H)
        # Pad slightly to ensure full erasure
        pad = 5
        cv2.rectangle(erased_img, (max(0, x-pad), max(0, y-pad)), (min(W, x+w+pad), min(H, y+h+pad)), (255, 255, 255), -1)
    ...
    for cnt in contours:
        ...
        if w * h < 2000: continue # Noise
        
        # This is a genuine diagram! It has survived full text erasure.
        # Let's pad it slightly to include nearby labels that we erased
        pad = 10
        dx = max(0, x - pad)
        dy = max(0, y - pad)
        dw = min(W - dx, w + 2*pad)
        dh = min(H - dy, h + 2*pad)
        diagram_boxes.append((dx, dy, dw, dh))
        
        y_center = dy + dh // 2
        crop_original = img[dy:dy+dh, dx:dx+dw]
        
        img_filename = f"{chapter_name}_{base_name}_diag_{diag_idx}.png"
        img_filepath = os.path.join(output_images_dir, img_filename)
        cv2.imwrite(img_filepath, crop_original)
    ...
    # 3. Process Text and Math
    texts_to_translate = []
    math_only_img = img.copy() # We will mask Chinese text on this to find math contours
    
    for block in ocr_blocks:
        rect = block["full_bbox"]
        bx = int(rect.origin.x * W)
        by = int((1.0 - rect.origin.y - rect.size.height) * H)
        bw = int(rect.size.width * W)
        bh = int(rect.size.height * H)
        b_area = bw * bh
        block_rect = (bx, by, bw, bh)
        
        # Check if inside diagram
        is_in_diagram = False
        for d_box in diagram_boxes:
            intersection = rect_intersection(block_rect, d_box)
            if intersection / b_area > 0.5: # 50% overlap
                is_in_diagram = True
                break
                
        if is_in_diagram:
            # Mask out from math_only_img so we don't process it later
            cv2.rectangle(math_only_img, (bx, by), (bx+bw, by+bh), (255, 255, 255), -1)
            continue
            
        # Outside diagrams.
        chinese_phrases = []
        full_y_center = by + bh // 2
        
        for c_data in block["chinese_data"]:
            crect = c_data["rect"]
            cx = int(crect.origin.x * W)
            cy = int((1.0 - crect.origin.y - crect.size.height) * H)
            cw = int(crect.size.width * W)
            ch = int(crect.size.height * H)
            
            # Mask Chinese chars with white on math_only_img
            cv2.rectangle(math_only_img, (max(0, cx-2), max(0, cy-2)), (min(W, cx+cw+2), min(H, cy+ch+2)), (255, 255, 255), -1)
    ...
    # 4. Extract Math
    # Mask diagrams completely on math_only_img so diagram contours don't show up as math
    for dx, dy, dw, dh in diagram_boxes:
        cv2.rectangle(math_only_img, (dx, dy), (dx+dw, dy+dh), (255, 255, 255), -1)
        
    mgray = cv2.cvtColor(math_only_img, cv2.COLOR_BGR2GRAY)
    mblurred = cv2.GaussianBlur(mgray, (5, 5), 0)
    medges = cv2.Canny(mblurred, 50, 150)
    mkernel = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 5)) # horizontal merge for math
    mdilated = cv2.dilate(medges, mkernel, iterations=2)
    mcontours, _ = cv2.findContours(mdilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    for cnt in mcontours:
        x, y, w, h = cv2.boundingRect(cnt)
        if w * h < 200: continue # Noise
        ...
        try:
            ...
            print("Pix2tex error:", e, flush=True)

    elements.sort(key=lambda e: e["y"])
    return elements

def build_chapter(chapter_name):
    print(f"Building {chapter_name}...", flush=True)
...
def main():
    ...
    for ch in chapters:
        print(f"Starting chapter {ch}", flush=True)
        build_chapter(ch)
